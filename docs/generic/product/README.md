...
// HCons(head=1000, tail=HCons(head=900, tail=arrow.generic.HNil@52b610d8))
```

```kotlin
Account(1000, 900).toHListLabeled()
// HCons(head=Tuple2(a=balance, b=1000), tail=HCons(head=Tuple2(a=available, b=900), tail=arrow.generic.HNil@52b610d8))
```
...
// DeferredK(deferred=LazyDeferredCoroutine{New}@e98c3c0, scope=kotlinx.coroutines.GlobalScope@5b0c4187)
```
